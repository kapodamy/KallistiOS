/* KallistiOS ##version##
   ...
   modified by kapodamy with the following changes:
        * replace snd_adpcm_split() calls with snd_pcm8_split() to improve performace (implies ADPCM stream is two-sample interlaced).
        * the function snd_stream2_alloc() ensures that the channels were allocated correctly.
*/
...
// Our stream structs
static strchan_t streams[SND_STREAM2_MAX];
...
void snd_stream2_filter_add(snd_stream_hnd_t hnd, snd_stream_filter_t filtfunc, void * obj) {
    filter_t * f;
...
void snd_stream2_filter_remove(snd_stream_hnd_t hnd, snd_stream_filter_t filtfunc, void * obj) {
    filter_t * f;
...
static void snd_stream_prefill_part(snd_stream_hnd_t hnd, uint32_t offset) {
    ...
    if(streams[hnd].stereo == 0) {
        ...
        if(streams[hnd].type == AICA_SM_8BIT || streams[hnd].type == AICA_SM_ADPCM_LS) {
...
int snd_stream2_init(void) {
    ...
    if(snd_init() < 0) {
        dbglog(DBG_ERROR, "snd_stream2_init(): snd_init() failed, giving up\n");
        return -1;
    }
...
snd_stream_hnd_t snd_stream2_alloc(snd_stream_callback_t cb, int bufsize) {
    ...
    /* Setup the callback */
    snd_stream2_set_callback(hnd, cb);
    ...
    if (streams[hnd].ch[0] == -1 || streams[hnd].ch[0] == -1) {
        printf("snd_stream2_alloc() failed, one or more channels could not be allocated\n");
        snd_stream2_destroy(hnd);
        return SND_STREAM_INVALID;
    }
...
snd_stream_hnd_t snd_stream2_reinit(snd_stream_hnd_t hnd, snd_stream_callback_t cb) {
    CHECK_HND(hnd);
...
static void snd_stream_start_type(snd_stream_hnd_t hnd, uint32 type, uint32 freq, int st) {
    ...
    chan->type = type;
    //chan->length = (streams[hnd].buffer_size / 2);// this field is never used
    chan->loop = 1;
    chan->loopstart = 0;
...
int snd_stream2_poll(snd_stream_hnd_t hnd) {
    ...
    if(ch0pos >= (streams[hnd].buffer_size / 2)) {
        dbglog(DBG_ERROR, "snd_stream2_poll: chan0(%d).pos = %ld (%08lx)\n", streams[hnd].ch[0], ch0pos, ch0pos);
        return -1;
    }
