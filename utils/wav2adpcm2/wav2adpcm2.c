...
void deinterleave_adpcm(void* buffer, size_t size) {
    uint8_t* src = (uint8_t*)buffer;
    uint8_t* dst = malloc(size);

    for (size_t i = 0, j = 0, hsize = size / 2; i < size; i += 2, j++) {
        dst[j] = src[i + 0];
        dst[j + hsize] = src[i + 1];
    }
...
void interleave_adpcm(void* buffer, size_t size) {
    uint8_t* src = (uint8_t*)buffer;
    uint8_t* dst = malloc(size);
...
int wav2adpcm(const char *infile, const char *outfile) {
    ...
    if(wavhdr.channels == 1) {
        ...
        /* For stereo deinterleave the input and store the
           left and right channel of the ADPCM data in pairs
           of two samples (LL RR LL RR). */
    ...
    wavhdr.format = 0x20; /* Yamaha ADPCM (ffmpeg) */
    wavhdr.byte_per_sec = (wavhdr.freq * wavhdr.bits * wavhdr.channels) / 8;
    wavhdr.bits = 4;
    wavhdr.blocksize = wavhdr.channels == 1 ? 1 : 2;// (wavhdr.channels * wavhdr.bits) / 8;
...
void usage() {
    printf("wav2adpcm2: 16bit mono/stereo wav to aica adpcm and vice-versa (c)2002 BERO (c)2023 kapodamy\n"
           "            this modified version generates two-samples interlaced ADPCM streams\n"
           "\n"
           ...
           " for stereo wav\n"
           "         ffmpeg -i input.wav -ac 2 -acodec pcm_s16le output.wav"
          );
}
