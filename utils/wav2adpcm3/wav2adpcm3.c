...
void deinterleave_adpcm(void* buffer, size_t size) {
    ...
    for (size_t i = 0, j = 0, hsize = size / 2; i < size; i += 2, j++) {
        ...
        uint8_t l1 = (b1 >> 4) & 0x0F;
        uint8_t l2 = (b2 >> 4) & 0x0F;
        uint8_t r1 = b1 & 0x0F;
        uint8_t r2 = b2 & 0x0F;
...
int adpcm2wav(const char *infile, const char *outfile) {
    ...
    if(validate_wav_header(&wavhdr, 0x20, 4, in)) {
        fclose(in);
        return -1;
    }
...
void usage() {
    printf("wav2adpcm3: 16bit mono/stereo wav to aica adpcm and vice-versa (c)2002 BERO (c)2023 kapodamy\n"
           "            this modified version generates real-world interlaced ADPCM streams\n"
           "\n"
